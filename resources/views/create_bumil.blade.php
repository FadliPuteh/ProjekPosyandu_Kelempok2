...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased">
        <div class="min-h-screen" style="background: #388CAB;">
            ...
            <!-- Page Content -->
            <x-guest-layout>
    <form method="POST" action="{{ route('store_bumil') }}">
        <p class="text-center">Pendaftaran Ibu Hamil</p>
        @csrf
        ...
        <!-- Jenis Imunisasi -->
        <div class="mt-4">
            <x-input-label for="usia_kandungan" :value="__('Usia Kandungan')" />
            <x-text-input id="usia_kandungan" class="block mt-1 w-full" type="text" name="usia_kandungan" :value="old('name')" required autocomplete="usia_kandungan" placeholder="Usia Kandungan, Contoh = 3 Bulan"/>
            <x-input-error :messages="$errors->get('usia_kandungan')" class="mt-2" />
        </div>
